...
float32_t Q_data[STATE_SIZE*STATE_SIZE] = {	0.4, 	0, 			0,				// q_theta
											...
											0,		0,			0.000001};		// q_bias
//measurement noise R
float32_t R_data[MEAS_SIZE*MEAS_SIZE] = {	6.5,	0,				//acc noise variance
										 	0,		1.44};			//gyro noise variance
...
void KalmanUpdate(){
	...
	arm_mat_mult_f32(&Eye, &P_minus_KHP, &P); // P_pred = P
